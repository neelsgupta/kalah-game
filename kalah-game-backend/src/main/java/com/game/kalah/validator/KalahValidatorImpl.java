...
public class KalahValidatorImpl implements KalahValidator {
	...
	public void validatePitId(Integer pitId) {
		...
		if (!(pitId > 0 && pitId < 14)) {
			log.error("Validation failed for pitId, entered pitId is out of bound. {} ", pitId);
			throw new InvalidIdException(String.valueOf(pitId),
					"Entered pitId is out of valid range, please enter valid pitId");
		}

		if (!(pitId != 7 && pitId != 14)) {
			log.error("Validation failed for pitId, entered pitId is one of kalah home pitid: {} ", pitId);
